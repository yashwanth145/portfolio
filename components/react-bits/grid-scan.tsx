...
export function GridScan({
  ...
  const pushScan = (time: number) => {
    ...
    if (materialRef.current) {
      ...
      for (let i = 0; i < arr.length && i < MAX_SCANS; i++) buffer[i] = arr[i];
  ...
  useEffect(() => {
    ...
    (uniforms.uLinesColor.value as THREE.Color).copy(srgbColor(linesColor));
    (uniforms.uScanColor.value as THREE.Color).copy(srgbColor(scanColor));
    ...
    uniforms.uScanDelay.value = Math.max(0.0, scanDelay);

    if (bloomRef.current) {
      bloomRef.current.blendMode.opacity.value = Math.max(0, bloomIntensity);
      (bloomRef.current as BloomEffect & { luminanceMaterial: { threshold: number; smoothing: number } })
